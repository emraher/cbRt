---
title: "Getting Started with cbRt"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Getting Started with cbRt}
...
The `cbRt` package provides an R interface to the Electronic Data Delivery System (EVDS) of the Central Bank of the Republic of Turkey (CBRT). This vignette will guide you through the basic usage of the package.
...
The data is returned as a tibble with a Date column and the series values.

### Retrieving Multiple Series

You can retrieve multiple series at once. The package will automatically handle different frequencies:
...
The EVDS API supports various data transformations through formulas. Available formulas:
...
The `cbrt_meta()` function retrieves metadata for all available series:
...
The EVDS API v3 has a limit of 150 observations per request. The `cbRt` package automatically handles this by:
...
By default, `cbrt_get()` returns a tibble, but you can specify other formats:
...
CBRT data sometimes includes "ND" (No Data) values. By default, these are converted to NA:
...
1. **Store your API token securely**: Use `.Renviron` rather than hardcoding it in scripts
2. **Check metadata first**: Use `cbrt_meta()` to find the correct series codes
3. **Be mindful of date ranges**: Very large date ranges will result in multiple API calls
4. **Use appropriate formulas**: Apply transformations at the API level when possible
...
- Package documentation: `?cbrt_get`, `?cbrt_meta`
- Report issues: [https://github.com/emraher/cbRt/issues](https://github.com/emraher/cbRt/issues)
- CBRT EVDS website: [https://evds3.tcmb.gov.tr/](https://evds3.tcmb.gov.tr/)
